...
const AddProductPage = () => {
  ...
  const fetchUniqueMainTags = useCallback(async () => {
    try {
      const res = await fetch('/api/product/get/unique-tags');
  ...
  const fetchSpecificCategoryData = useCallback(async () => {
    ...
    try {
      ...
      const resVariant = await fetch(`/api/product/get/get-specific-category-variant/${variantId}`);
      if (!resVariant.ok) {
      ...
      const resCategory = await fetch(`/api/product/get/get-specific-category/${variantData.specificCategory}`);
      if (!resCategory.ok) {
  ...
  const fetchLatestProduct = useCallback(async () => {
    if (specificCategoryVariant) {
      try {
        const res = await fetch(`/api/product/get/get-reference?variantCode=${specificCategoryVariant.variantCode}`);
